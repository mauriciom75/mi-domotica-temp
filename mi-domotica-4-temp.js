          ...
          client.socket.setNoDelay(true);
              ...
              client.socket.on('close', function() {
                  ...
                  setTimeout(function () {
                    ...
                    client.connect(4000, argv.ardu_host, function() {
              ...
              client.socket.on('data', function(data) {
                ...
                var id_invocacion = valores[0];
                if ( tipo=="D" && id_invocacion != "" && id_invocacion != "0") {

		  if (typeof lista_respuesta[id_invocacion] != 'undefined' && !lista_respuesta[id_invocacion].cumplido)
		  {
                    // ejecuto el call back
                    lista_respuesta[id_invocacion].funcion(tipo,valores);
		  }
                }

//                lista_respuesta[id_invocacion] = { 'timestamp' : Date.now() , 'funcion' : crearFuncionRespuesta() }; 
...
function procesarMensajeGenerico(client, datArdu)
{
    if (datArdu.tipo=='E')
    {
        ...
        if (datArdu.valores[1]=='1' &&
            datArdu.valores[2]=='1' &&
            datArdu.valores[3]=='101' )
        {


	var options = {
	    url : 'http://192.168.1.5:4001/api/variables/boton1_1', 
	    method : 'PATCH',
	    json : true,
            body : {"type":"value","valueOrExpression":datArdu.valores[0]},
 	    headers: {
	      'Authorization': 'Basic ' + new Buffer("admin" + ':' + "48274827").toString('base64')
            }   
	};
...
//    const resourcePromise = myPool.acquire()
    ...
    resourcePromise3.then( function(client) {
            ...
            str = 'H' + id_invocacion + ',' + req.params.controlador_id + ',' + req.params.nroDisp + ',' + '\r';
            console.log(str);
            client.socket.write(str);
...
//-----------------------------------
app.get('/controladores/:controlador_id/dispositivos/:nroDisp/3estado', function (req, res){
    var micb={};
    var str;
...
//    const resourcePromise = myPool.acquire()
    ...
    resourcePromise3.then( function(client) {
            ...
            function crearFuncionRespuesta() {

                return function (tipo,valores) {

                    console.log("entré a la función de respuesta.");

                    if (tipo == 'D')
                    {
                        lista_respuesta[id_invocacion]['cumplido'] = true;
			...
			{
                            estado = dispositivo.estado;
                            res.status(200).send(estado.toString());
			}
			else
			{
                            estado = 1;
                            res.status(500).send(estado.toString());
                    	}
                    }
                }
            }

            lista_respuesta[id_invocacion] = { 'timestamp' : Date.now() , 
                                                'funcion' : crearFuncionRespuesta() , 
                                                'pool_client' : client ,
                                                'cumplido' : false }; 
...
//-----------------------------------
app.get('/controladores/:controlador_id/dispositivos/:nroDisp/2datos', function (req, res){
    var micb={};
    var str;
...
//-----------------------------------
app.get('/controladores/:controlador_id/dispositivos/:nroDisp/display/:texto', function (req, res){
    var micb={};
    var str;
...
//    const resourcePromise = myPool.acquire()
    ...
    resourcePromise3.then( function(client) {
            ...
            str = 'L' + id_invocacion + ',' + req.params.controlador_id + ',' + req.params.nroDisp + ',' + req.params.texto + ',' + '\r';
            console.log(str);

            client.socket.write(str, function () {
                res.sendStatus(200);
		clearTimeout(tout);
                setTimeout( function () {
                    myPool.release(client);
...
function limpiar_lista_respuesta() {
    var keys = Object.keys(lista_respuesta);
    var timestamp_actual = Date.now();

    console.log("limpiar lista ids a verificar: " + keys);

    keys.forEach(function (key) {
        if ( lista_respuesta[key].cumplido == true 
	     && lista_respuesta[key].timestamp + 10000 < timestamp_actual ) {
            console.log("borro respuesta cumplida (10s): " + key);
            delete lista_respuesta[key];
        } else {
   	  // si pasó mas de un minuto
	  if ( lista_respuesta[key].timestamp + 70000 < timestamp_actual ) {
	        console.log("borro respuesta x timeout: " + key);
	        //myPool.release(lista_respuesta[key].pool_client);
	        delete lista_respuesta[key];
	  }
        }
    })

    setTimeout(limpiar_lista_respuesta,5000);
}
